...
> Resolved 2026-09-17: this file previously contained unresolved, nested git
> conflict markers (two overlapping merges, `af4545e` and `laraxot/dev`). The
> narrative changelog entries below are kept; a raw, undated Pest stack-trace
> dump that was on the losing side of the conflict was dropped as a stale
> generated artifact rather than documentation (see
> `git-conflict-mass-resolution` guidance: generated output is deleted, not
> merged). Run `XDEBUG_MODE=coverage vendor/bin/pest Modules/Job --coverage`
> for current coverage numbers; none are asserted here as ground truth.
